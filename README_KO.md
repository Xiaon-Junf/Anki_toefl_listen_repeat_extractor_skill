...
TOEFL 스피킹 시험 오디오 녹음 파일과 PDF, DOCX 또는 TXT 형식의 정답 파일을 가지고 있는데, 파일을 자주 전환하고 여러 창을 열어야 해서 장기적으로 따라말하기 문제 연습을 유지하기 어렵다면 이 기술을 시도해보세요!

그냥 Agent 에게 이렇게 말하면 됩니다: `TOEFL 스피킹 따라말하기 문제의 정답은 xxx.pdf 에 있고, 오디오는 xxx.mp3 입니다. 정답을 입력하는 Anki 플래시카드를 만들어 주세요.` Agent 가 작동 방법을 알고 있으며, 배치 모드도 지원합니다. 그런 다음 CSV 파일 하나만 Anki 에 가져오면, 개인의 망각 곡선에 따라 언제 어디서나 실제 시험 문제로 듣기와 따라말하기 연습을 할 수 있습니다. 간단하죠? 시도해보세요!

아래 도구를 직접 사용하고 싶지만 프로그래밍 배경이 없는 경우, `Readme.md` 와 `SKILL.md` 를 Agent 에게 보여주세요. 대화 중에 요구사항을 확인하고 CSV 파일을 생성해 줍니다.

그래도 사용할 수 없는 경우, 저자가 이미 추출한 CSV 파일과 오디오 클립 (업데이트 예정, 클라우드 스토리지 사용) 을 참조하여 Anki 에 직접 가져오세요.
